...
var postLog = (level, message, jwt_token) => {
  var xmlhttp = new XMLHttpRequest();
  xmlhttp.open("POST", "http://localhost:1337/api/logs");
  xmlhttp.setRequestHeader("Content-Type", "application/json");
  xmlhttp.setRequestHeader("Authorization", "Bearer " + jwt_token);
  xmlhttp.send(
    JSON.stringify({
      data: {
        message,
...
function App() {
  const [jwt_token, setJwtToken] = React.useState("");
  ...
  const postLogInfo = (message) => {
    postLog("INFO", message, jwt_token);
  };

  const postLogDebug = (message) => {
    postLog("DEBUG", message, jwt_token);
  };

  const postLogError = (message) => {
    postLog("ERROR", message, jwt_token);
  };
    ...
    <div className="App">
      ...
      <div style={{ padding: "1rem" }}>
        <input
          ...
          placeholder={"setJwtToken"}
        />

        <button
          onClick={(e) => {
            postLogInfo("post info log from client");
          }}
        >
          postLogInfo
        </button>
        <button
          onClick={(e) => {
            postLogDebug("post debug log from client");
          }}
        >
          postLogDebug
        </button>
        <button
          onClick={(e) => {
            postLogError("post error log from client");
          }}
        >
          postLogError
        </button>
